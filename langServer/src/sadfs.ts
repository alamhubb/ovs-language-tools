...
export const qqsLanguagePlugin: LanguagePlugin<URI> = {
  ...
  typescript: {
    extraFileExtensions: [{ extension: 'qqs', isMixedContent: true, scriptKind: 7 satisfies ts.ScriptKind.Deferred }],
    getServiceScript() {
      return undefined;
    },
...
export class qqsVirtualCode implements VirtualCode {
  ...
  constructor(public snapshot: ts.IScriptSnapshot) {
    ...
    this.embeddedCodes = [...getqqsEmbeddedCodes(snapshot, this.htmlDocument)];
  }
}

function* getqqsEmbeddedCodes(snapshot: ts.IScriptSnapshot, htmlDocument: html.HTMLDocument): Generator<VirtualCode> {
